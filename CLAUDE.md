...
Use Claude Code as the single local work console. Use the OpenAI Codex plugin through explicit `/codex:*` commands for Codex drafting, adversarial review, code review, and bounded investigation.

Before implementation, read the active branch docs under `docs/branches/<branch-doc-dir>/`, especially `README.md` and `plans/next-agent-handoff.md`.

When the user asks to implement a feature or fix, use `/implement-with-review` to orchestrate Codex implementation followed by parallel Codex and Claude sub-agent review. Do not implement directly without review unless the user explicitly says to skip it.

When the user asks to plan, design, spec out, or investigate work, use `/plan-with-codex` to orchestrate Codex investigation and plan drafting. Do not write plans directly without Codex investigation unless the user explicitly says to skip it.
